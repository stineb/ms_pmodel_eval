...
\begin{tabular}{lllllll}
  ...
  \midrule
  FULL\_DT & 0.74 & 0.69 & 0.70 & 0.72 & 0.28 & 0.08 \\ 
  FULL\_NTsub & 0.75 & 0.70 & 0.70 & 0.73 & 0.28 & 0.09 \\ 
  FULL\_Ty & 0.73 &  &  & 0.71 & 0.44 &  \\ 
  \bottomrule
  \end{tabular}
...
\begin{tabular}{lllllll}
  ...
  \midrule
  FULL\_DT & 1.88 & 390.45 & 363.55 & 1.78 & 1.58 & 155.03 \\ 
  FULL\_NTsub & 1.87 & 418.65 & 392.20 & 1.75 & 1.59 & 150.76 \\ 
  FULL\_Ty & 1.76 &  &  & 1.71 & 1.28 &  \\ 
  \bottomrule
  \end{tabular}
...
Most of the explanatory power of the different models for predicting annual total GPP stems from their power in predicting between-site (``spatial'') variations (Fig. \ref{fig:modobs_spatialannual}). The \rsq\ for annual GPP simulated by the P-model setups ranges from 0.57 (ORG) to 0.7 (FULL). The NULL model achieves an \rsq\ of 0.58. The \rsq\ for spatial variations ranges from 0.62 (ORG) to 0.7 (FULL), and 0.64 for the NULL model. In contrast, inter-annual (between years) variations are poorly simulated (\rsq : 0.06-0.09 for P-model setups, and 0.04 for the NULL model). This is also illustrated by Fig. \ref{fig:modobs_spatialannual}. For a perfect fit between modelled and observed annual GPP values, all black lines (representing the linear regression model of annual values for a single site) would lie on the 1:1 line and have a slope of 1. The fact that for some sites, the linear regression has a slope that deviates much from 1, or is even negative, is illustrative of the poor model performance in capturing interannual variability. Within site variations in annual GPP (interannual variations) are generally much smaller than across site variations, which likely adds to the challenge of capturing interannual variations. Interannual GPP variations are generally better captured at sites where the variability is high and in particular at dry sites (site-level plots in SI XXX). 

Interannual variations are better simulated when using MODIS EVI as a forcing. However, spatial variations are much better captured when using MODIS FPAR. 
...
\includegraphics[width=0.45\textwidth]{fig/meandoy_byzone_Dfc_north_greenness.pdf}\\
\includegraphics[width=0.45\textwidth]{fig/meandoy_byzone_Cfb_north_greenness.pdf}
\includegraphics[width=0.45\textwidth]{fig/meandoy_byzone_Aw_south_greenness.pdf}
